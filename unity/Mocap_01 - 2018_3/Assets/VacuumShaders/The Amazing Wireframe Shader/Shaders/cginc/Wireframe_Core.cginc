...
half _V_WIRE_Size;
	...
	float _V_WIRE_DynamicMaskType; //0 - Smooth, 1 - Solid
	float _V_WIRE_DynamicMaskSmooth;
#endif
...
float _V_WIRE_DynamicGI;
float _V_WIRE_DynamicGIStrength;
float3 _V_WIRE_ObjectWorldPos;
	...
	#else
		#define EMISSION_STRENGTH (_V_WIRE_DynamicGI > 0.5 ? _V_WIRE_DynamicGIStrength : 0);
	#endif
#else
	#define EMISSION_STRENGTH _V_WIRE_EmissionStrength;
#endif
...
inline half ExtructWireframeFromMass(half3 mass, float fixedSize)
{
	...
	#else
		float size = _V_WIRE_Size / (1 + fixedSize * _V_WIRE_FixedSize);
...
inline half WireOpaque(fixed4 wireTexColor, inout fixed4 srcColor, fixed3 mass, float wireFade, float fixedSize, half maskValue, out float3 emission)
{
	half value = ExtructWireframeFromMass(mass, fixedSize);
		...
		value = lerp(value, 1, maskValue);
				...
				srcColor.rgb = lerp(lerp(srcColor.rgb, FIXED_3_ONE, maskValue), lerp(FIXED_3_ONE, srcColor.rgb, maskValue), _V_WIRE_DynamicMaskEffectsBaseTexInvert);
			#endif
		#endif 
	#endif 
...
inline half WirCutout(fixed4 wireTexColor, inout fixed4 srcColor, fixed3 mass, float wireFade, float fixedSize, half maskValue, half customAlpha, half cutoff, out float3 emission)
{
	...
	fixed fade = lerp(1, saturate(wireFade), _V_WIRE_DistanceFade);
	...
	half clipValue = 1;
		...
		emission = 0;
	#else
		emission = wColor.rgb * (1 - value) * EMISSION_STRENGTH;
		#ifdef V_WIRE_TRANSPARENCY_ON
...
{
	float smooth = 0.2 + _V_WIRE_DynamicMaskSmooth * 4 + _V_WIRE_DynamicMaskType * 2048;

	#if defined(V_WIRE_DYNAMI_MASK_PLANE)

		half gValue = saturate(dot(_V_WIRE_DynamicMaskWorldNormal * smooth * _V_WIRE_DynamicMaskInvert, normalize(maskPos - _V_WIRE_DynamicMaskWorldPos)));
		
		return gValue;

	#elif defined(V_WIRE_DYNAMIC_MASK_SPHERE)

		half gValue = 1 - (distance(maskPos, _V_WIRE_DynamicMaskWorldPos) / _V_WIRE_DynamicMaskRadius);
		gValue = saturate(gValue * smooth * _V_WIRE_DynamicMaskInvert * -1);
