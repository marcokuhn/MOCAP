...
#define UNITY_APPLY_FOG_PBR(coord,col)  UNITY_CALC_FOG_FACTOR((coord).x); UNITY_FOG_LERP_COLOR(col,fixed4(0,0,0,0),unityFogFactor)
#else
...
void surf (Input IN, inout SurfaceOutputStandard o) 
{
	...
	retColor.rgb *= lerp(FIXED_3_ONE, IN.color.rgb, _V_WIRE_VertexColor);
	...
	o.Occlusion = lerp(1, LerpOneTo(tex2D (_V_WIRE_AoMap, IN.texcoord.xy).g, _V_WIRE_AoStrength), _V_WIRE_Ao);
		...
		//Mask
			...
			wireTexColor.rgb *= lerp(FIXED_3_ONE, IN.color.rgb, _V_WIRE_WireVertexColor);
